...
addpath(genpath('.../CoverTrees'));
addpath(genpath('.../NearestNeighbors'));
...
if isfield(opts,'ReuseOpts') && (opts.ReuseOpts)
    ...
    if ~isfield(opts,'CoverTreeOpts') || isempty(opts.CoverTreeOpts)          
        opts.CoverTreeOpts = struct('theta',0.5,'numlevels',int32(100),'minlevel',int32(0), 'BLOCKSIZE',int32(1024),'distancefcn',opts.distancefcn);
    end
end
...
lGoWithIdxs = isa(cXq,'int32') || isa(cXq,'uint32');

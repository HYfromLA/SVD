function [EstDim, NoiseSigma] = EstDim_MSVD( X, parallel, cOpts )
...
%
% OUT:
%   EstDim      : estimated intrinsic dimension of the data: either a number or a vector of numbers if pointwise estimates are requested
...
if ~cOpts.AllPtsFast
    ...
    if parallel
    parfor k = 1:cOpts.NumberOfTrials
        Nets = [];
        ...
        if cOpts.PtIdxs==0
            ...
            [lEstDimStats(k), NoiseSigma(k)]  = EstimateDim_LargestMultiscaleGap( X, struct('Nets',Nets,'NetsOpts',NetsOpts,'Stats',MSVD_Stats), struct('Verbose',cOpts.verbose,'Ptwise',cOpts.Ptwise) );
...
EstDim=floor(EstDim); NoiseSigma = sqrt(mean(NoiseSigma));
